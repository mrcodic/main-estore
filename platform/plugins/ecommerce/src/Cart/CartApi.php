...
class CartApi extends Cart
{
     ...
     */
    public function add($id, $name = null, $qty = null, $price = null, array $options = [], $identifierAsToken = null ,$customer=null)
    {
        ...
        $identifier = $identifierAsToken;

        if ($this->storedCartWithIdentifierExists($identifier) || $customer) {
            $cart = $this->getCartExists($identifier, $customer)->first();
            $content = $cart ? unserialize($cart->content) : $this->getContent();
        }else{
        ...
        $cartsCustomer = $customer ? $this->getCartExists(null, $customer)->first(): false;

        if($cartsCustomer):
            $identifier = $cartsCustomer->identifier ;
            $carts = $this->getCartExists($identifierAsToken);
            $carts->whereNull('customer_id')->delete();
        endif;

        $identifier = $identifier ?? substr(base_convert(sha1(uniqid(mt_rand())), 16, 36), 0, 30);

        $this->store($identifier, $content , $customer);

        $this->events->dispatch('cart.added', $cartItem);


        return [
                    'token_cart' => $identifier,
                    'items' => $content,
                ];
    }
     ...
     * @return void
     */
    public function store($identifier ,$content, $customer=null, $restore=false)
    {

        $cart = $this->getCartExists( $identifier, $customer);

        if($cart){
            $restore ? $this->restore($cart->first()->identifier) :null;
    ...
    protected function storedCartWithIdentifierExists(string $identifier=null): bool
    {
        return $this->getConnection()->table($this->getTableName())->where('identifier', $identifier)->exists();
    }

    protected function getCartExists(string $identifier=null, $customer=null)
    {
        return $this->getConnection()->table($this->getTableName())
                    ->when(fn($q) => $q->whereNotNull('customer_id')->where('customer_id', $customer))
