...
class BostaWebhookController extends BaseController
{
    ...
    public function index(Request $request, BaseHttpResponse $response)
    {
        ...
        switch ($event) {
            ...
            case 'track_updated':
                ...
                break;
            default:
                $this->bosta->log([__LINE__, print_r($request->input(), true)]);
        ...
        if (! $shipment) {
            $this->bosta->log([__LINE__, print_r($condition, true)]);
    ...
    protected function transactionUpdated(Shipment $shipment, array $data)
    {
        ...
        $this->shipmentHistoryRepository->createOrUpdate([
            ...
            'description' => trans('plugins/bosta::bosta.transaction.updated', [
    ...
    protected function trackUpdated(Shipment $shipment, array $data)
    {
        ...
        $this->shipmentHistoryRepository->createOrUpdate([
            ...
            'description' => trans('plugins/bosta::bosta.tracking.statuses.' . Str::lower($status)),
            'order_id' => $shipment->order_id,
            'user_id' => 0,
            'shipment_id' => $shipment->id,
        ]);
