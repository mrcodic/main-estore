...
use Botble\Bosta\Services\Abstracts\Bosta as GoBosta;
use Botble\Bosta\Services\Abstracts\Bosta_Error;
use Botble\Bosta\Services\Abstracts\Bosta_Rate;
use Botble\Bosta\Services\Abstracts\Bosta_Transaction;
use Botble\Bosta\Services\Abstracts\Shipment as Bosta_Shipment;
use Botble\Bosta\Services\Abstracts\Order as Bosta_Order;
...
class Bosta
{
    ...
    {
        $this->liveApiToken = setting('shipping_bosta_production_key');
        // $this->testApiToken = setting('shipping_bosta_test_key');
        $this->labelFileType = 'PDF';
        // $this->sandbox = setting('shipping_bosta_sandbox', 1) == 1;

        // if ($this->sandbox) {
        //     GoBosta::setApiKey($this->testApiToken);
        // } else {
            GoBosta::setApiKey($this->liveApiToken);
        // }

        // dd(GoBosta::getApiKey());
        ...
        $this->packageTypes = config('plugins.bosta.general.package_types', []);
        $this->serviceLevels = config('plugins.bosta.general.service_levels', []);

        $this->useCache = setting('shipping_bosta_cache_response', 1);

        $this->logging = setting('shipping_bosta_logging', 1);

        $this->cache = new Cache(app('cache'), self::class);

        $this->logger = Log::channel('bosta');
    }
    ...
    public function validate(): array
    {
        ...
        $apiTokenName = trans('plugins/bosta::bosta.live_api_token');
        if ($this->sandbox) {
            $apiTokenName = trans('plugins/bosta::bosta.test_api_token');
        }

        if (! $this->getApiKey()) {
            $errors[] = trans('plugins/bosta::bosta.token_name_is_required', ['name' => $apiTokenName]);
        } elseif (! $this->validateActiveApiToken()) {
            $errors[] = trans('plugins/bosta::bosta.token_name_is_invalid', ['name' => $apiTokenName]);
        }
    ...
    public function getRates(array $params, bool $suggest = true, bool $order = false): array
    {
        ...
        if (! Arr::get($newResponse, 'shipment.rates', []) && $suggest && Arr::get($prepareParams, 'extra.COD', [])) {
            ...
            $suggestResponse = $this->getCacheOrNewRates($suggestParams, $order);
    ...
    public function getCacheOrNewRates(array $params, bool $order = false): array
    {
        ...
        if (! $response) {
            if (! $params['address_from'] || ! $params['address_to']) {
                ...
                if ($order) {
                    ...
                    $response = $this->calculatorShipment($requestParams);
                }
            }
        } else {
        ...
        if ($order) {
            ...
            $result = $this->getRatesResponse($response, $params);
        }

        return $result;
    }

    public function calculatorShipment(array $params)
    {
        $cacheKey = $this->getCacheKey($params);
        $response = $this->getCacheValue($cacheKey);
        if (! $response || !Arr::get($response, 'rates')) {
            ...
            try {
                $response = Bosta_Shipment::get($params);

                if (Arr::get($response, 'success')) {
                    $params['rates'] = Arr::get($response, 'rates', [[
                        'servicelevel'=>[
                            'token'=>$response['data']['tier']['_id'],
                            'name'=>"Bosta"
                        ],
                        'object_id'=>$response['data']['tier']['_id'],
                        'provider'=>'Bosta',
                        'provider_image_75'=>'https://docs.bosta.co/img/logo.svg',
                        'price'=>$response['data']['priceAfterVat']
                    ]]);
                    $params['status'] = 'SUCCESS';
                    $params['object_created'] = Carbon::now()->toIso8601String();
                    $params['object_id'] = $response['data']['tier']['_id'];
                    $params['object_owner'] = $params['address_from']['email'];
    ...
    public function createShipment(array $params)
    {
        ...
        if (!Arr::get($params, 'rates') || !$response || !Arr::get($response, 'rates')) {
            $params = $this->calculatorShipment( $params );
        }
        ...
        // try {
            ...
            if (Arr::get($response, 'success')) {
                ...
                $params['object_updated']   = Carbon::now()->toIso8601String();
                $params['shipment_date']    = Carbon::now()->toIso8601String();
                $params['shipment_id']      = $response['data']['_id'];
                $params['tracking_id']      = $response['data']['trackingNumber'];
    ...
    public function getPrepareParams(array $inParams): array
    {
        ...
        $params['order_total']   = Arr::get($inParams, 'order_total');
        $params['parcels']       = $this->prepareParcelInfo($inParams);
        $params['items']         = Arr::get($inParams, 'items', []);
        $params['payment_method']= Arr::get($inParams, 'payment_method', 'cod');
    ...
    protected function getRatesParams(array $inParams): array
    {
        ...
        $params['COD'] = (float)Arr::get($inParams, 'order_total', 0);

        if ($cod = Arr::get($inParams, 'extra.COD', [])) {
            $params['extra']['COD'] = $cod;
            $params['COD'] = (float)$cod['amount'];
        }
        ...
        // [Normal, Light Bulky, Heavy Bulky]
        $params['size'] = "Normal";
        $params['type'] = "CASH_COLLECTION";
        // $params['type'] = "SEND";

        // if ($payment_method = Arr::get($inParams, 'payment_method','bank_transfer')) {
        //     // [SEND, CASH_COLLECTION, CUSTOMER_RETURN_PICKUP, EXCHANGE, SIGN_AND_RETURN].
        //     $params['type'] = $payment_method == 'cod' ? "CASH_COLLECTION" :"SEND";
        // }
        $params['allowToOpenPackage'] = false;
    ...
    protected function getDeleiveryParams(array $inParams): array
    {
        $itemsCount = 0;

        foreach ($inParams['items'] as $item) {
        ...
        foreach ($inParams['rates'] as $rate) {
            $priceAfterVat += $rate['price'];
        }

        $inParams['cod'] = (float)$inParams['COD'] + $priceAfterVat;

        $inParams['specs'] = [
            // "size" =>  "MEDIUM",//$inParams['size'],
            "packageType" =>  'Parcel',
            "packageDetails" => [
        ...
        $inParams['dropOffAddress'] = [
            "city"       =>  $inParams['dropOffCity'],
            "districtId" =>  $inParams['address_to']['district_id'],//$inParams['districtId'],
            "firstLine"  =>  $inParams['address_to']['address'],
        ];

        // $inParams['pickupAddress'] = [
        //     "city"       =>  $inParams['dropOffCity'],
        //     "districtId" =>  $inParams['address_to']['district_id'],//$inParams['districtId'],
        //     "firstLine"  =>  $inParams['address_to']['address'],
        // ];
        ...
        // $inParams['pickupAddress'] = [
        //     "city"       =>  $inParams['pickupCity'],
        //     "districtId" =>  $inParams['districtId'],
        //     "firstLine"  =>  $inParams['address_to'],

        // ];

        $inParams['receiver'] = [
            "firstName" =>  $inParams['address_to']['name'],
            "phone"     =>  $inParams['address_to']['phone'],
            "email"     =>  $inParams['address_to']['email'],
        ];
    ...
    protected function afterPrepareAddress(array $addr): array
    {
        if (EcommerceHelper::loadCountriesStatesCitiesFromPluginLocation()) {
            ...
            // dd($city->state->slug);
            $state = Location::getStateNameById($addr['state']);
            if ($city) {
                $addr['city'] = $city->name;
                // $addr['state'] = $city->state->abbreviation;
                $addr['state'] = $state;
                $addr['country'] = $city->state->country->code;
                $addr['gov_id'] = $city->state->gov_id;
                $addr['district_id'] = $city->district_id;
            }
        }
    ...
    protected function getShipmentResponse($response, array $params): array
    {
        ...
        foreach (Arr::get($response, 'rates', []) as $rate) {
            ...
            if ($days) {
                $description = trans('plugins/bosta::bosta.estimated_days', ['day' => $days]);
            } else {
    ...
    public function canCreateTransaction(Shipment $shipment): bool
    {
        ...
        if ($order && $order->id && $order->shipping_method->getValue() == BOSTA_SHIPPING_METHOD_NAME
    ...
    {
        try {
            $transaction = Bosta_Transaction::create([
                'rate' => $rateId,
                'async' => false,
            ])->__toArray(true);
    ...
    public function retrieveRate(string $rateId)
    {
        ...
        $response = $this->getCacheValue($cacheKey);
        if (! $response) {
            $response = Bosta_Rate::retrieve($rateId)->__toArray(true);
            $this->setCacheValue($cacheKey, $response);
        }
    ...
    public function retrieveShipment(string $shipmentId)
    {
        ...
        if (! $response) {
            try {
                $response = Bosta_Shipment::retrieve($shipmentId)->__toArray(true);
                $this->setCacheValue($cacheKey, $response);
            } catch (Bosta_Error $ex) {
