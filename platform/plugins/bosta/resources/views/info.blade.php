    ...
    <div class="row">
        <div class="col-12 my-3 text-center">
            <div>
                ...
                <div>
                    <small
                        class="text-secondary">{{ trans('plugins/bosta::bosta.estimated_days', ['day' => $days]) }}</small>
                </div>
            </div>
        </div>
        <div class="col-12 my-2">
            @include('plugins/bosta::address')
        </div>
        ...
        <div class="col-12 my-2">
            <div class="row">
                ...
                <div class="col-6">
                    @if ($cod = Arr::get($shipmentBosta, 'extra.COD'))
                        <span
                            class="fw-bold"
                        ...
                        <table class="table">
                            ...
                            <tr>
                                <td>{{ trans('plugins/bosta::bosta.order_amount') }}</td>
        ...
        <div class="col-12 mt-2">
            <span class="fw-bold fs-5">{{ trans('plugins/bosta::bosta.parcel') }}</span>
            @php
                $parcel = Arr::get($shipmentBosta, 'parcels.0');
        ...
        @php
            $url = route('ecommerce.shipments.bosta.transactions.create', $shipment->id);
            $isShowButton = true;
            if (!is_in_admin(true)) {
                if (is_plugin_active('marketplace')) {
                    $url = route('marketplace.vendor.orders.bosta.transactions.create', $shipment->id);
                }
                ...
                <div class="col-12 my-3">
                    <div class="alert alert-warning">
                        <small>
                            ...
                            <span>{{ trans('plugins/bosta::bosta.note_5') }}</span>
                        </small>
                    </div>
                    <button
                        class="btn btn-primary get-new-rates"
                        data-url="{{ route('ecommerce.shipments.bosta.rates', $shipment->id) }}"
                        type="button"
                    >
                        {{ trans('plugins/bosta::bosta.recheck_rate') }}
                    </button>
                </div>
            @endif
        @endif
